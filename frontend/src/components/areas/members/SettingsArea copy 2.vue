<template>
	<!-- 區塊：個人設定－始 -->
	<div v-if="user" class="selectContent" id="settings" style="display: none;">
		<!-- 🌈 個人化設定 -->
		<section class="mb-4">
			<h5>個人化設定</h5>
			<div class="soild"></div>
		...
		<section id="infoSection" class="mb-4 edit-section">
			...
			<div class="inp_modle">
				<label>暱稱</label>
				<input type="text" v-model="user.nickname" class="edit-input" style="display:none;">
				<p class="display-text">{{ user.nickname }}</p>
			</div>
			<div class="inp_modle">
				<label class="form-label">性別</label>
				<select v-model="user.gender" class="edit-input form-select" style="display:none;">
					<option>男性</option>
					<option>女性</option>
					<option>隱藏</option>
				</select>
				<p class="display-text">{{ user.gender }}</p>
			</div>
			<div class="inp_modle">
				<label class="form-label">生日</label>
				<input type="date" v-model="formattedBirth" class="edit-input" style="display:none;">
				<p class="display-text">{{ formattedBirth }}</p>
			</div>
			<div class="inp_modle">
				<label class="form-label">個人簡介</label>
				<textarea v-model="user.summary" class="edit-input" style="display:none; resize: none;"></textarea>
		...
		<section id="contactSection" class="mb-4 edit-section">
			...
			<div class="inp_modle towline">
				...
				<div class="d-flex align-items-center w-100">
					<input type="email" v-model="user.email" class="edit-input" style="display:none;">
			...
			<div class="inp_modle towline">
				...
				<div class="d-flex align-items-center w-100">
					<input type="text" v-model="user.phone" class="edit-input" style="display:none;">
					<p class="display-text">{{ user.phone }}</p>
			...
			<div class="inp_modle towline">
				...
				<div class="d-flex flex-column align-items-center w-100">
					<div class="old row w-100">
						<input type="text" v-model="user.address" class="edit-input" style="display:none;">
						<p class="display-text">{{ userAddress }}</p>
					</div>
...
export default {
	...
	async mounted() {
		...
		try {
			...
			console.log('取得會員資料成功', this.user);

			// 避免 null，塞預設值
			if (!this.user.summary) {
				this.user.summary = '字裡行間，自有清歡。願以此生，與君共賞。';
			}
			if (!this.user.address) {
				this.user.address = '未設定地址';
			}
	...
	computed: {
		...
		formattedBirth: {
			get() {
				return this.user?.birth ? dayjs(this.user.birth).format('YYYY-MM-DD') : '';
			},
			set(val) {
				this.user.birth = val ? dayjs(val).toISOString() : '';
			}
		},
		userSummary() {
			return this.user.summary && this.user.summary.trim() !== ''
				? this.user.summary
				: '字裡行間，自有清歡。願以此生，與君共賞。';
		},
		userAddress() {
			return this.user.address && this.user.address.trim() !== ''
				? this.user.address
				: '未設定地址';
		},
	},
